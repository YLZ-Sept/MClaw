...
function _getCredentials() {
  const { getVideoCredentials } = require('../routes/model-configs');
  return getVideoCredentials('chanjing');
}

function _baseUrl() {
  return _getCredentials().baseUrl.replace(/\/+$/, '');
}
...
async function _getToken() {
  ...
  const resp = await fetch(`${_baseUrl()}/open/v1/access_token`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ app_id: creds.appId, secret_key: creds.secretKey }),
...
async function listPublicDp(page = 1, size = 20, tagIds) {
  const url = new URL(`${_baseUrl()}/open/v1/list_common_dp`);
...
async function listPublicAudio(page = 1, size = 20) {
  const resp = await fetch(`${_baseUrl()}/open/v1/list_common_audio?${new URLSearchParams({ page, size })}`, {
    headers: await _headers(), signal: AbortSignal.timeout(15000)
  });
  return _handle(await resp.json());
}
...
async function listFonts() {
  const resp = await fetch(`${_baseUrl()}/open/v1/font_list`, {
    headers: await _headers(), signal: AbortSignal.timeout(15000)
  });
  return _handle(await resp.json());
}
...
async function createVideo(params) {
  const resp = await fetch(`${_baseUrl()}/open/v1/create_video`, {
...
async function getVideo(videoId) {
  const resp = await fetch(`${_baseUrl()}/open/v1/video?${new URLSearchParams({ id: videoId })}`, {
    headers: await _headers(), signal: AbortSignal.timeout(15000)
  });
  return _handle(await resp.json());
}
...
async function listVideos(page = 1, size = 20) {
  const resp = await fetch(`${_baseUrl()}/open/v1/video_list`, {
...
async function createTTS(params) {
  const resp = await fetch(`${_baseUrl()}/open/v1/create_audio_task`, {
    method: 'POST', headers: await _headers(), body: JSON.stringify(params),
    signal: AbortSignal.timeout(15000)
  });
  return _handle(await resp.json()); // { task_id }
...
async function getTTS(taskId) {
  const resp = await fetch(`${_baseUrl()}/open/v1/audio_task_state`, {
...
async function createLipSync(params) {
  const resp = await fetch(`${_baseUrl()}/open/v1/video_lip_sync/create`, {
    method: 'POST', headers: await _headers(), body: JSON.stringify(params),
    signal: AbortSignal.timeout(30000)
  });
  return _handle(await resp.json()); // task_id
...
async function getLipSync(id) {
  const resp = await fetch(`${_baseUrl()}/open/v1/video_lip_sync/detail?${new URLSearchParams({ id })}`, {
    headers: await _headers(), signal: AbortSignal.timeout(15000)
  });
  return _handle(await resp.json());
}
...
async function listLipSync(page = 1, size = 20) {
  const resp = await fetch(`${_baseUrl()}/open/v1/video_lip_sync/list`, {
    method: 'POST', headers: await _headers(), body: JSON.stringify({ page, page_size: size }),
    signal: AbortSignal.timeout(15000)
  });
  return _handle(await resp.json());
...
async function getUploadUrl(service, name) {
  ...
  const resp = await fetch(`${_baseUrl()}/open/v1/common/create_upload_url?${new URLSearchParams(params)}`, {
    headers: await _headers(), signal: AbortSignal.timeout(15000)
  });
  return _handle(await resp.json());
}
...
async function listTags(businessType) {
  const url = new URL(`${_baseUrl()}/open/v1/common/tag_list`);
...
async function deleteVideo(id) {
  const resp = await fetch(`${_baseUrl()}/open/v1/delete_video`, {
...
async function getUserInfo() {
  const resp = await fetch(`${_baseUrl()}/open/v1/user_info`, {
    headers: await _headers(), signal: AbortSignal.timeout(15000)
  });
  return _handle(await resp.json());
}
...
async function getUserDuration() {
  const resp = await fetch(`${_baseUrl()}/open/v1/user_duration`, {
    headers: await _headers(), signal: AbortSignal.timeout(15000)
  });
  return _handle(await resp.json());
}
