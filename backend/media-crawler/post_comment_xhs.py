...
async def find_comment_input(page):
    ...
    strategies = [
        ...
        ("placeholder", lambda: page.get_by_placeholder("友善评论")),
    ...
    for name, factory in strategies:
        try:
            ...
            if await loc.count() > 0:
                utils.logger.info(f"[post_comment_xhs] 输入框命中: {name}")
                return loc
        except Exception:
            continue
    ...
    for loc in generic:
        try:
            if await loc.count() > 0:
                utils.logger.info("[post_comment_xhs] 输入框命中: 通用定位器")
                return loc
        except Exception:
            continue

    # Tier 3: 硬编码兜底（小红书常见 DOM）
    fallback = [
        ...
        "[class*='note-comment'] textarea",
        "[class*='note-comment'] input",
        "[class*='reply'] textarea",
        "[class*='reply'] input[type='text']",
        "#comment-input",
    ]
    for sel in fallback:
        try:
            ...
            if await loc.count() > 0:
                utils.logger.info(f"[post_comment_xhs] 输入框命中: 兜底 {sel}")
                return loc
        except Exception:
            continue
...
async def find_submit_button(page):
    ...
    for name in ["发送", "发布", "评论", "提交"]:
        try:
            ...
            if await btn.count() > 0:
                utils.logger.info(f"[post_comment_xhs] 按钮命中: role=button, name={name}")
                return btn
        except Exception:
            continue
    ...
    for text in ["发送", "发布", "提交"]:
        try:
            ...
            if await btn.count() > 0:
                utils.logger.info(f"[post_comment_xhs] 按钮命中: text={text}")
                return btn
        except Exception:
            continue
    ...
    for btn in generic_btns:
        try:
            if await btn.count() > 0:
                utils.logger.info("[post_comment_xhs] 按钮命中: 通用定位器")
                return btn
        except Exception:
            continue
    ...
    for sel in fallback:
        try:
            ...
            if await btn.count() > 0:
                utils.logger.info(f"[post_comment_xhs] 按钮命中: 兜底 {sel}")
                return btn
        except Exception:
            continue
...
async def ensure_logged_in(browser_context, page):
    ...
    if not web_session:
        ...
        utils.logger.info("[post_comment_xhs] 登录完成")

    return True


async def post_comment_flow(page, note_url, content):
    """导航到笔记页并发布评论"""
    utils.logger.info(f"[post_comment_xhs] 打开笔记: {note_url}")
    await page.goto(note_url, wait_until="domcontentloaded")
    await asyncio.sleep(5)  # XHS 页面加载较慢

    # 尝试关闭可能的弹窗
    try:
        close_btns = page.locator("[class*='close']").first
        if await close_btns.count() > 0:
            await close_btns.click(timeout=2000)
    ...
    if not input_box:
        # 尝试点击评论区区域触发输入框
        utils.logger.info("[post_comment_xhs] 未找到输入框，尝试点击评论区...")
        click_targets = [
            page.get_by_text("评论"),
            page.get_by_text("说点什么..."),
            page.locator("[class*='comment'] span").first,
            page.locator("[class*='note-comment']").first,
        ]
    ...
    if not input_box:
        utils.logger.error("[post_comment_xhs] 无法找到评论输入框")
    ...
    await asyncio.sleep(1)
    utils.logger.info(f"[post_comment_xhs] 已输入评论: {content[:50]}...")
    ...
    if submit_btn:
        ...
        await asyncio.sleep(3)
        utils.logger.info("[post_comment_xhs] 评论已提交")
        return {"success": True, "method": "click"}
    else:
        utils.logger.info("[post_comment_xhs] 未找到发送按钮，尝试 Enter 键...")
...
async def main():
    ...
    if not note_id or not content:
        print("Usage: python post_comment_xhs.py <note_id> <content>")
        sys.exit(1)
