...
MClaw/
  ...
  backend/
    ...
    routes/                   ← 路由文件 (见下方路由表)
    services/                 ← 业务逻辑模块 (见下方服务表)
    agents/                   ← 内置 Agent 工具定义
      executor.js             ← 核心工具执行器（含 search_skills/load_skill/execute_command）
      internal.js / sales.js / support.js / bid.js / hot-video.js / import-parser.js
      vector-search.js        ← FAQ 向量相似搜索
    scripts/
      install-mateclaw-skills.js ← 一键安装 29 个 mateclaw 技能
    channels/                 ← 多渠道消息接入
      index.js                ← 统一消息处理中心
  ...
  frontend/
    ...
    src/
      main.js                 ← Vue 入口 + 全局错误捕获 (error-reporter)
      ...
      layouts/MainLayout.vue  ← 侧边栏 + 全局 WebSocket 初始化
      components/
        ChatMessage.vue       ← Markdown 消息气泡 + 下载链接实时替换
        ImportDialog.vue      ← 数据导入弹窗
      composables/
        useWebSocket.js       ← 全局 WebSocket 单例 (替代各模块轮询)
      utils/
        error-reporter.js     ← 前端全局错误捕获 + 上报
      views/                  ← 页面 (见下方路由表)
...
| `/api/clawhub` | `routes/clawhub.js` | ClawHub 市场 + 技能管理 |
| `/api/errors/report` | server.js 内联 | 前端错误上报 (公开) |
...
| `/api/wiki` | `routes/wiki.js` | LLM Wiki 页面 CRUD + AI 消化 |
...
| `/api/download` | `routes/downloads.js` | 文件下载 + download_logs 记录 |
...
| `services/health-pusher.js` | WebSocket 系统健康推送 (每30s) |
| `services/wiki-digest.js` | Wiki 知识库 AI 消化引擎 |
...
1. `loadAgentConfig(agent)` 加载系统提示词 + 工具 + 技能
2. 提示词注入用户消息开头，`config.tools` 传给 LLM API
3. LLM 可通过 search_skills → load_skill → execute_command 按需调用技能

### 模式 3：通用聊天 (无 agent)
1. `loadAgentConfig('*')` 加载全局通配符技能 + 默认提示词
2. 注入 system 消息 + `config.tools` 传给 LLM API
3. LLM 可主动搜索并执行任何可用技能

> 三条路径均支持技能调用：search_skills(搜索) → load_skill(加载指令) → execute_command(执行)
...
- **招投标:** bid 查询/统计
- **财务:** 应收/应付
- **技能发现:** search_skills / load_skill / execute_command (按需加载+执行)
...
2. 注入额外工具：文件操作、定时任务、技能按需发现(search_skills/load_skill/execute_command)、知识库注入
...
**AI:** `agent_apps` (含 kb_ids/skill_bindings/wiki_page_ids), `agent_openclaw_skills`, `model_configs`, `chat_sessions`, `chat_messages`, `kb_articles`, `kb_categories`, `wiki_pages` (含 kb_id), `wiki_sources`, `wiki_links`, `wiki_page_versions`, `wikihub_kbs`, `wikihub_materials`, `skill_translations`, `skill_usage_log`, `faq`
**追爆款:** `hot_products`, `hot_contents`, `hot_conversations`, `hot_leads`, `content_publish`, `rewrite_history`
**其他:** `social_tasks`, `social_comments`, `social_replies`, `social_monitors`, `auto_reply_rules`, `auto_reply_logs`, `asset_ledger`, `download_logs`
...
### 技能按需发现系统
- 全磁盘扫描 `~/.openclaw/workspace/skills/` + `~/.openclaw/skills/` + `~/.agents/skills/`
- 系统提示词仅注入轻量目录（名称+描述），不加载完整 SKILL.md
- `search_skills(query)`：LLM 按关键词搜索可用技能
- `load_skill(name)`：LLM 按需加载指定技能的完整使用说明
- `execute_command(command)`：在 OpenClaw 节点执行技能 CLI 命令
- 三条聊天路径（数字员工/专家 Agent/通用聊天）全部支持
- 全局通配符 `agent_id='*'` 绑定 11 个通用技能

### 知识库架构 (kb_id 统一隔离)
- `wiki_pages.kb_id` 替代 `category` 字段混用方案
- `wikihub_kbs` 作为多知识库注册中心
- Agent 编辑表单支持多选 KB → 运行时自动加载该 KB 下全部 Wiki 页面
- WikiHub 支持上传整个文件夹并逐一 AI 消化

### WebSocket 健康推送
- `health-pusher.js`：每 30s 广播 `system_health`（服务状态+CPU/内存+渠道健康）
- `useWebSocket.js`：全局单例 + 自动重连 + 响应式 ref
- ServiceManagement / MessageChannels 删除 HTTP 轮询，改为 WebSocket 订阅

### 前端错误监控
- `error-reporter.js`：Vue/Promise/JS/Router 全捕获
- 5 分钟去重 + 面包屑追踪 + sendBeacon 上报
- `POST /api/errors/report` 公开端点 → `logs` 表

### 文档下载全链路
- ChatMessage 实时替换 localhost + workspace 路径为可点击下载链接
- `download_logs` 表记录每次下载 (username + filename + type)
- `GET /api/download/history` 按用户查询
- Workspace 文件 24h 过期自动清理

### 技能库 UI
- 对标 ExpertHub 设计：Hero 统计 + 分类胶囊 + 彩色技能卡片
- 三个标签页：我的技能 / ClawHub 市场 / 使用统计
- 技能详情弹窗显示依赖关系（被哪些 Agent 引用）
- 内置/管理技能隐藏导出卸载按钮；workspace 技能可管理
- 版本更新检查对比本地 vs ClawHub
- Agent 编辑表单集成 kb_ids + skillBindings 多选
## 开发惯例
